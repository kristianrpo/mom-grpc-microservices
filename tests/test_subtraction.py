...
def test_inmediate_response():
    """
    Should return the result of the subtraction service immediately when it is running.
    """
    proc = start_process("subtraction_service.py", "micro_services/micro_service_subtraction")
    
    try:
        cmd = build_submit_command("test_client", "SubtractionService", 1, 2)
        ...
        assert data["subtraction"] == -1.0, f"❌ Expected difference is -1.0, but got {data['subtraction']}"
        print(f"✅ Test passed with status {data['status']} and difference {data['subtraction']}")
...
def test_delayed_response_from_queue():
    ...
    proc_service = start_process("subtraction_service.py", "micro_services/micro_service_subtraction")
    ...
    try:
        ...
        if match:
            ...
            assert data["status"] == "COMPLETED", f"❌ Expected status is COMPLETED, but got {data['status']}"
            assert data["result"] == -1.0, f"❌ Expected difference is -1.0, but got {data['result']}"
