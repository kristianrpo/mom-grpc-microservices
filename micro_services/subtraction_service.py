...
import proto.subtraction_service.subtraction_service_pb2 as subtraction_service_pb2
import proto.subtraction_service.subtraction_service_pb2_grpc as subtraction_service_pb2_grpc
import redis
import json
import time
import threading
...
class SubtractService(SubtractionServiceServicer):
    ...
    def SubtractNumbers(self, request, context):
        """
        Subtract two numbers provided in the request and returns the result in JSON format.
        ...
        Returns:
            SubtractNumbersResponse: The response message containing the result in JSON.
        """
        try:
            ...
            return subtraction_service_pb2.SubtractNumbersResponse(result=json_result)
...
def process_tasks(redis_handler):
    ...
    while True:
        ...
        task_json = redis_handler.pop_next_task(service="SubtractionService")
        if task_json:
            try:
...
def serve():
    ...
    server.start()
    print(f"🚀 SubtractionService gRPC server started on port {port}")

    # Optionally, publish the service status to Redis.
    redis_handler.publish_service_status("SubtractionService", True)
